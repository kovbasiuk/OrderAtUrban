...
class  RTData : ObservableObject {
    ...
    init(){
        ...
        //calls getMenu from MenuItemHelper to download menu items from firestore and initialises to to an array based on menu item categories
        MenuItemHelper.shared.getMenu {
            self.categoriesData = MenuItemHelper.shared.menuItems
                    }
    ...
    func sendOrder(order: Order) {
        ...
        //reset curremt order after its been sent to firestore
        self.currOrder = Order(items: [MenuItem:Int](), orderID: UUID().uuidString)
        ...
        db.collection("orders").addSnapshotListener({
            ...
            for doc in snap!.documentChanges{
                ...
                if !doc.document.data().isEmpty {
                    ...
                    //new orders get appended with no ammendments made
                    if doc.type == .added{
